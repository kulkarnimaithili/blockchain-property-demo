...
var options = 	{
					...
					chaincode:{
						zip_url: 'https://github.com/aarongoa/blockchain-property-demo/archive/v1.0.zip',
						unzip_dir: 'blockchain-property-demo-1.0/chaincode',													//subdirectroy name of chaincode after unzipped
						git_url: 'http://gopkg.in/aarongoa/blockchain-property-demo.v1/chaincode',						//GO get http url
...
ibc.load(options, function (err, cc){														//parse/load chaincode, response has chaincode functions!
	if(err != null){
		...
		if(deployement_check()) {					//yes, go deploy
			cc.deploy('init', ['99'], {save_path: 'chaincode_details', delay_ms: 30000}, function(e){ 						//delay_ms is milliseconds to wait after deploy for conatiner to start, 50sec recommended
...
// Checks if deployment is required or not; If not, i.e. already deployed, assigns chaincode hash to chaincode instance
function deployement_check() {
	const dir = '/chaincode_details';
  var files = fs.readdirSync(__dirname + dir);
  for (var i in files) {
    if (files[i].includes('.json')) {
			const chaincode_hash = files[i].replace('.json','');
	    chaincode.details.deployed_name = chaincode_hash;
      return false;
    }
  }
  return true;
...
function check_if_deployed(e, attempt){
	if(e){
		...
		chaincode.query.readInit(['abc'], function(err, resp){
			...
			try{
				if(err == null){															//no errors is good, but can't trust that alone
					if(resp === 'null') cc_deployed = true;									//looks alright, brand new, no marbles yet
						...
						if(json === 99) cc_deployed = true;					//looks alright, we have marbles
					}
				}
			}
